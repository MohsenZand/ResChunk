...
H36M_MAJOR_JOINTS = [0, 1, 2, 3, 4, 6, 7, 8, 9,
                     11, 12, 13, 14, 16, 17, 18, 19, 24, 25, 26, 27]
H36M_NR_JOINTS = 32
H36M_PARENTS = [-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14,
...
class ForwardKinematics(object):
    ...
    def from_quat(self, joint_angles):
        ...
        """
        qs = quaternion.from_float_array(
            np.reshape(joint_angles, [-1, H36M_NR_JOINTS, 4]))
...
class H36MForwardKinematics(ForwardKinematics):
    ...
    def __init__(self):
        ...
        super(H36MForwardKinematics, self).__init__(offsets, H36M_PARENTS, norm_idx=7, left_mult=True, major_joints=H36M_MAJOR_JOINTS)
...
class Visualizer(object):
    ...
    def __init__(self, db, interactive, rep="rotmat", fk_engine=None,
                 output_dir=None, skeleton=True, dense=True, to_video=False, save_figs=False):
        """
    ...
    def create_clip_skeleton(self, joint_angles, title):
        ...
        fps = 60 if self.db == 'amass' else 50
        fps = fps * 4
...
def animate_matplotlib(positions, colors, titles, fig_title, parents, change_color_after_frame=None, color_after_change=None, overlay=False, fps=60, out_dir=None, to_video=True, fname=None, save_figs=False, interactive=True):
    ...
    axes = [fig.add_subplot(n_axes, 1, i + 1, projection='3d') for i in range(n_axes)]
    # axes = [fig.add_subplot(1, i + 1, n_axes, projection='3d') for i in range(n_axes)]
